...
from octopus_user.models import UserRecommendedBasket, UserGeneratedBasket, UserSettings
...
def port_basket(request):
    ...
    rb_product_list = list()
    rb_quantity_list = list()
    for product in recommended_basket:
        rb_product_list.append(product['id'])
        rb_quantity_list.append(product['quantity'])

    user_recommended_basket = UserRecommendedBasket(user=user,
                                                    product_list=rb_product_list,
                                                    quantity_list=rb_quantity_list)
    ...
    ugb_product_list = list()
    ugb_quantity_list = list()
    for product in user_generated_basket:
        ugb_product_list.append(product['id'])
        ugb_quantity_list.append(product['quantity'])

    user_recommended_basket = UserGeneratedBasket(user=user,
                                                  product_list=ugb_product_list,
                                                  quantity_list=ugb_quantity_list,
                                                  user_recommended_basket=user_recommended_basket)
    user_recommended_basket.save()
    ...
    for product in user_generated_basket:
        user_generated_basket_before_porting.append(
            (Product.objects.get(id=product['id']), str(int(product['quantity']))))

    thread_manager = ThreadManager()
    this_basket = BasketToPort(request, email, password,
                               user_generated_basket_before_porting, thread_manager)
    ...
    user_generated_basket_after_porting = check_basket_persistence(
        user_generated_basket_before_porting, user_generated_basket_after_porting)

...
def create_onboarding_basket(user, data=None):
    ...
    tags_id = get_list_from_comma_separated_string(
        user_settings.tags)
...
def get_or_create_later_basket(user):
    ...
    ugb = UserGeneratedBasket.objects.filter(
        user=user).order_by('created_at')
    urb = UserRecommendedBasket.objects.filter(
        user=user).order_by('created_at')

     # no basket generated by user yet, just create an anonymous one.
    if not ugb:
        basket, __ = create_onboarding_basket(user)
        #if I have already created a recommendation:
        if urb:
